...
int main() {
    int n, num, score, count_eq_num = 0;
    cin >> n;
    vector<pair<int, int>> ss(n);
    for (int i = 0; i < n; i++) {
        cin >> num >> score;
        ss[i] = {score, -num}; //создание пары баллы - номер
    }

    sort(ss.begin(), ss.end()); //сортировка
    reverse(ss.begin(), ss.end());
    
    for (auto elm : ss) { //вывод
        cout << -elm.second << " " << elm.first << endl;
    }
    return 0;
}